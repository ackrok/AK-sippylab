...
nDrug = 2; % CHANGE
nMouse = numel(unique({comb.mouse}));
injTime = 20; totTime = 80;
clr = [0.0 0.5 0.75; 1 0.25 0.0; 1 0.75 0.25];
...
for i = 1:length(comb)
    ...
    mask = 1:(injTime+2) * 60 * comb(1).beh.calibrate.frameRate;
    distance(mask) = nan; 
    distPost = sum(distance, 'omitnan');
    timePost = numel(find(~isnan(distance))) ./ comb(i).beh.calibrate.frameRate;
    vec(i) = distPost/timePost;
end
for k = 1:nDrug
    total = [total, vec(k:nDrug:end)'];
end
...
for n = 1:length(uni)
    ...
    plot(binTime, out{n}./100, '-x');
    xlabel('time (min)'); 
    ylabel('distance (m per 1 min)'); ylim([0 10]); yticks(0:5:20);
    legend({'saline','ket 30','meth 10'});
    title(uni{n});
end
...
for k = 1:nDrug
    ...
    idx = arrayfun(@(x) find(mask(:,x)), 1:nDrug, 'UniformOutput', false);
...
legend({'saline','','ketamine 30'});

subplot(1,2,2); hold on
b = bar(1:nDrug, mean(total)); b.FaceColor = [0.8 0.8 0.8]; b.EdgeColor = 'none';
errorbar(1:nDrug, mean(total), SEM(total,1), 'k', 'LineWidth', 2);
plot(total','*-', 'LineWidth', 1);
xticks(1:nDrug); xticklabels({'saline','ketamine 30','meth 10'});
