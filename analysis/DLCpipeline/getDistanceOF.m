function beh = getDistanceOF(beh)
...
%       beh.tbl.x - x coordinates, in cm (vector)
%       beh.tbl.y - y coordinates, in cm (vector)
%       beh.tbl.likelihood - confidence (vector)
%
% Outputs:
...
%%
coor = [beh.tbl.x, beh.tbl.y];
likelihood = beh.tbl.likelihood;
...
numBins = ceil(numel(x) / framesPerBin); % number of bins, ensure integer
...
distPerBin = accumarray(binIdx, distance(:), [numBins, 1], @nansum, 0); % cm per bin
% Identify bins that had zero non-NaN elements and set them to NaN
countValid = accumarray(binIdx, ~isnan(distance(:)), [numBins,1], @sum, 0);
distPerBin(countValid == 0) = nan;  % choose NaN to indicate no valid data
